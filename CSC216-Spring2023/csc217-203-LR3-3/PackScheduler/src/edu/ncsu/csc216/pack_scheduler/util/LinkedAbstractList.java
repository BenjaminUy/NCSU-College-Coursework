...
public class LinkedAbstractList<E> extends AbstractList<E> {
	...
	public void add(int index, E element) {
		...
		// Allow back to reference the last LinkNode
		while (current != null) {
			current = current.next;
		}
		// reset current for list traversal
		current = front;
		if (index == 0 && size() == 0) {
			front = new ListNode(element); // If size is 0 and index is 0, the front is set to a new ListNode
		}
		if (index == 0) {
		...
		} else if (index == size) {					// Errors are happening over here whenever I try to use back field
