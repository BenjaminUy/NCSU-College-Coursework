...
/**
 * Tests class for RegistrationManager class
 * @author Benjamin
 * @author Hunt
 * @author Andrew 
 */
public class RegistrationManagerTest {
	...
	/**
	 * Tests getCourseCatalog method for an empty catalog
	 */
	@Test
	...
	/**
	 * Tests getStudentDirectory method for an empty directory
	 */
	@Test
	...
	public void testGetFacultyDirectory() {
		...
		fd.addFaculty("Noah", "Anderson", "neander2", "neander2@ncsu.edu", "pw", "pw", 3);
		assertEquals(1, fd.getFacultyDirectory().length);
		
	}
	/**
	 * Tests login functionality for correct student and password and for an incorrect password
	 */
	@Test
	public void testLogin() {
		...
		manager.getFacultyDirectory().addFaculty("Laura", "Pilsner", "lpilsner", "lpilsner@ncsu.edu", "pw", "pw", 3);
		manager.getStudentDirectory().addStudent("Noah", "Anderson", "neander2", "neander2@ncsu.edu", "pw", "pw", 3);
		manager.logout();
		assertTrue(manager.login("neander2", "pw"));
		manager.logout();
		assertTrue(manager.login("lpilsner", "pw"));
		manager.logout();
		Exception e1 = assertThrows(IllegalArgumentException.class, () -> manager.login("htynch", "pw"));
	...
	/**
	 * Tests getCurrentUser method by testing the current user against user credentials in property list
	 */
	@Test
	...
	public void testAddFacultyToCourse() {
		...
		manager.login(registrarUsername, registrarPassword);
		assertTrue(manager.addFacultyToCourse(cc.getCourseFromCatalog("CSC116", "001"), fd.getFacultyById("kpatel")));
		assertTrue(manager.removeFacultyFromCourse(cc.getCourseFromCatalog("CSC116", "001"), fd.getFacultyById("kpatel")));
		manager.resetFacultySchedule(fd.getFacultyById("kpatel"));
	}
}