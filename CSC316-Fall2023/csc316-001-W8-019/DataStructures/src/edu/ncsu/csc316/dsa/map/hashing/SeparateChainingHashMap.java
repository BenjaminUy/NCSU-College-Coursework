...
public class SeparateChainingHashMap<K extends Comparable<K>, V> extends AbstractHashMap<K, V> {
    ...
    public void createTable(int capacity) {
        ...
        table = new RedBlackTreeMap[capacity];
        size = 0;
    }
    ...
    public V bucketPut(int hash, K key, V value) {
    	Map<K, V> bucket = table[hash];
    	if (bucket == null) {
    		table[hash] = new RedBlackTreeMap<K, V>();
    		bucket = table[hash];
    	}
    	int previousSize = bucket.size();
    	V original = bucket.put(key, value);
    	
    	if (previousSize < bucket.size()) {
    		size++;
    	}
    	return original;
