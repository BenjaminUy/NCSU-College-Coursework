...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E> implements BinaryTree<E> {
    ...
    public int numChildren(Position<E> p) {
    	int totalChildren = 0;
    	// Check if given position exists in Tree
    ...
    // TODO Look into sibling method, may have issues with choosing the non-sentinel node if starting
    // with a sentinel node first
    ...
    public Position<E> sibling(Position<E> p) {
        ...
        // If the given Position is the root, p has no siblings
        if (parent == null) {
        	return null;
        }
        
        // At this point, the parent must have at least one child, so we must find the Position
