...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
        ...
        ALEdge ale = validate(e);
        
        Vertex<V>[] vArray = ale.getEndpoints();
        ALVertex v1 = validate(vArray[0]);
        ALVertex v2 = validate(vArray[1]); 
        
        if (this.isDirected()) {
        	ale.setOutgoingListPosition(v1.getOutgoing().addLast(ale));
        	ale.setIncomingListPosition(v2.getIncoming().addLast(ale));

        } else {
        	ale.setIncomingListPosition(v1.getIncoming().addLast(ale));	
        	ale.setOutgoingListPosition(v2.getIncoming().addLast(ale));	
    ...
    public void removeEdge(Edge<E> edge) {
    	...
    	ALVertex alv1 = validate(v[0]);
    	ALVertex alv2 = validate(v[1]);
    	
    	Position<Edge<E>> outgoingPosition = e.getOutgoingListPosition();
    	Position<Edge<E>> incomingPosition = e.getIncomingListPosition();
       
        if (this.isDirected()) {
        	alv1.getOutgoing().remove(outgoingPosition);
        	alv2.getIncoming().remove(incomingPosition);
        } else {
        	alv1.getIncoming().remove(incomingPosition);
        	alv1.getIncoming().remove(outgoingPosition);
        	alv2.getIncoming().remove(incomingPosition);
        	alv2.getIncoming().remove(outgoingPosition);
        }
    }
     ...
     * 
     * @param e Edge to cast into an adjacency list edge
     ...
     * 
     * @param v Vertex to cast into an adjacency list vertex
