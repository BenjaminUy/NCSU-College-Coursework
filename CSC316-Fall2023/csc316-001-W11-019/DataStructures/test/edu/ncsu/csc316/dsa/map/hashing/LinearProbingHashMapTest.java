package edu.ncsu.csc316.dsa.map.hashing;

import static org.junit.Assert.*;
import java.util.Iterator;
import java.util.NoSuchElementException;

import org.junit.Before;
import org.junit.Test;
import edu.ncsu.csc316.dsa.map.Map;

/**
 * Test class for LinearProbingHashMap
 * Checks the expected outputs of the Map abstract data type behaviors when using
 * a linear probing hash map data structure 
 *
 * @author Dr. King
 * @author Benjamin Uy
 */
public class LinearProbingHashMapTest {

    private Map<Integer, String> map;

    /**
     * Create a new instance of a linear probing hash map before each test case executes
     */     
    @Before
    public void setUp() {
        // Use the "true" flag to indicate we are testing.
        // Remember that (when testing) alpha = 1, beta = 1, and prime = 7
        // based on our AbstractHashMap constructor.
        // That means you can draw the hash table by hand
        // if you use integer keys, since Integer.hashCode() = the integer value, itself
        // Finally, apply compression. For example:
        // for key = 1: h(1) = ( (1 * 1 + 1) % 7) % 7 = 2
        // for key = 2: h(2) = ( (1 * 2 + 1) % 7) % 7 = 3
        // for key = 3: h(3) = ( (1 * 3 + 1) % 7) % 7 = 4
        // for key = 4: h(4) = ( (1 * 4 + 1) % 7) % 7 = 5
        // for key = 5: h(5) = ( (1 * 5 + 1) % 7) % 7 = 6
        // for key = 6: h(6) = ( (1 * 6 + 1) % 7) % 7 = 0
        // etc.
        map = new LinearProbingHashMap<Integer, String>(7, true);
    }
    
    /**
     * Test the output of the put(k,v) behavior
     */     
    @Test
    public void testPut() {
        assertEquals(0, map.size());
        assertTrue(map.isEmpty());
        
        assertNull(map.put(1, "string1")); 				// Should be at index 2 of table
        assertEquals(1, map.size());
        
        assertEquals("string1", map.put(1, "uno"));		// Should be at index 2 of table
        assertEquals(1, map.size());
        
        assertNull(map.put(3, "string3"));				// Should be at index 4 of table
        assertNull(map.put(8, "string8"));				// Should be at index 3 of table
        assertNull(map.put(2, "string4"));				// Should be at index 5 of table
        assertEquals(4, map.size());
        
        Iterator<Map.Entry<Integer, String>> it = map.entrySet().iterator();
        assertEquals(1, (int) it.next().getKey());
        assertEquals(8, (int) it.next().getKey());
        assertEquals(3, (int) it.next().getKey());
        assertEquals(2, (int) it.next().getKey());
        
        assertNull(map.put(6, "string6"));				// Should be at index 0 of table
        assertEquals(5, map.size());
        
        it = map.entrySet().iterator();
        assertEquals(6, (int) it.next().getKey());
        assertEquals(1, (int) it.next().getKey());
        assertEquals(8, (int) it.next().getKey());
        assertEquals(3, (int) it.next().getKey());
        assertEquals(2, (int) it.next().getKey());
    }
    
    /**
     * Test the output of the get(k) behavior
     */     
    @Test
    public void testGet() {
        assertTrue(map.isEmpty());
        assertNull(map.get(1));
        assertNull(map.put(1, "string1"));
        assertEquals("string1", map.get(1));
        
        assertEquals("string1", map.put(1, "uno"));
        assertEquals("uno", map.get(1));
    }
    
    /**
     * Test the output of the remove(k) behavior
     */ 
    @Test
    public void testRemove() {
        assertTrue(map.isEmpty());
        assertNull(map.remove(1));
        assertNull(map.put(1, "string1"));
        assertEquals(1, map.size());
        
        assertEquals("string1", map.remove(1));
        assertEquals(0, map.size());
        assertTrue(map.isEmpty());
    }
    
    /**
     * Test the output of the iterator() behavior, including expected exceptions
     */     
    @Test
    public void testIterator() {
        Iterator<Integer> it = map.iterator();
        try {
        	it.next();
        } catch (Exception e) {
        	assertTrue(e instanceof NoSuchElementException);
        }
        
        assertNull(map.put(1, "string1"));
        assertNull(map.put(2, "string2"));
        assertNull(map.put(3, "string3"));
        assertNull(map.put(8, "string8"));
        
        it = map.iterator();
        assertTrue(it.hasNext());
        assertEquals(1, (int) it.next()); // Should be at index 2
        assertTrue(it.hasNext());
        assertEquals(2, (int) it.next()); // Should be at index 3
        assertTrue(it.hasNext());
        assertEquals(3, (int) it.next()); // Should be at index 4
        assertTrue(it.hasNext());
        assertEquals(8, (int) it.next()); // Should be at index 5
        assertFalse(it.hasNext());
    }
    
    /**
     * Test the output of the entrySet() behavior
     */     
    @Test
    public void testEntrySet() {
        Iterator<Map.Entry<Integer, String>> it = map.entrySet().iterator();     
        try {
        	it.next();
        } catch (Exception e) {
        	assertTrue(e instanceof NoSuchElementException);
        }
        
        assertNull(map.put(1, "string1"));
        assertNull(map.put(2, "string2"));
        assertNull(map.put(3, "string3"));
        assertNull(map.put(8, "string8"));
        
        it = map.entrySet().iterator();
        assertTrue(it.hasNext());
        assertEquals(1, (int) it.next().getKey()); // Should be at index 2
        assertTrue(it.hasNext());
        assertEquals(2, (int) it.next().getKey()); // Should be at index 3
        assertTrue(it.hasNext());
        assertEquals(3, (int) it.next().getKey()); // Should be at index 4
        assertTrue(it.hasNext());
        assertEquals(8, (int) it.next().getKey()); // Should be at index 5
        assertFalse(it.hasNext());
        
    }
    
    /**
     * Test the output of the values() behavior
     */  
    @Test
    public void testValues() {
        Iterator<String> it = map.values().iterator();
        try {
        	it.next();
        } catch (Exception e) {
        	assertTrue(e instanceof NoSuchElementException);
        }
        
        assertNull(map.put(1, "string1"));
        assertNull(map.put(2, "string2"));
        assertNull(map.put(3, "string3"));
        assertNull(map.put(8, "string8"));
        
        it = map.values().iterator();
        assertTrue(it.hasNext());
        assertEquals("string1", it.next()); // Should be at index 2
        assertTrue(it.hasNext());
        assertEquals("string2", it.next()); // Should be at index 3
        assertTrue(it.hasNext());
        assertEquals("string3", it.next()); // Should be at index 4
        assertTrue(it.hasNext());
        assertEquals("string8", it.next()); // Should be at index 5
        assertFalse(it.hasNext());
    }
    
    /**
     * Test the output of the various LinearProbingHashMap constructor methods
     */
    @Test
    public void testLinearProbingMap() {
    	// Test the default constructor
    	map = new LinearProbingHashMap<Integer, String>();
    	assertEquals(0, map.size());
    	assertTrue(map.isEmpty());
    	map.put(1, "string1");
    	map.put(2, "string2");
    	map.put(3, "string3");
    	assertEquals(3, map.size());
    	assertFalse(map.isEmpty());
    	map.remove(2);
    	assertEquals(2, map.size());
    	assertFalse(map.isEmpty());
    	
    	// Test constructor with boolean isTesting parameter
    	map = new LinearProbingHashMap<Integer, String>(true);
    	assertEquals(0, map.size());
    	assertTrue(map.isEmpty());
    	map.put(1, "string1");
    	map.put(2, "string2");
    	map.put(3, "string3");
    	assertEquals(3, map.size());
    	assertFalse(map.isEmpty());
    	map.remove(2);
    	assertEquals(2, map.size());
    	assertFalse(map.isEmpty());
    	
    	// Test constructor with integer capacity parameter
    	map = new LinearProbingHashMap<Integer, String>(2);
    	assertEquals(0, map.size());
    	assertTrue(map.isEmpty());
    	map.put(1, "string1");
    	map.put(2, "string2");
    	map.put(3, "string3");
    	assertEquals(3, map.size());
    	assertFalse(map.isEmpty());
    	map.remove(2);
    	assertEquals(2, map.size());
    	assertFalse(map.isEmpty());
    }
}