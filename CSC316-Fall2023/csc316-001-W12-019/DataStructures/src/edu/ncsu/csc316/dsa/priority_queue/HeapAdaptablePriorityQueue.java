...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
     ...
     * Code is based off implementation provided in CSC 316 textbook on adaptable priority queues.
     */
    @Override
    public void remove(Entry<K, V> entry) {
        AdaptablePQEntry<K, V> temp = validate(entry);
        int location = temp.getIndex();
        if (location == list.size() - 1) {
        	list.remove(list.size() - 1);
        } else {
        	swap(location, list.size() - 1);
        	list.remove(list.size() - 1);
        	bubble(location);
        }
    }
    ...
    /**
     * Replaces the given Entry's key with the given key
     * Code is based off implementation provided in CSC 316 textbook on adaptable priority queues.
     */
    @Override
    ...
    /**
     * Removes the given Entry's value with the given value
     * Code is based off implementation provided in CSC 316 textbook on adaptable priority queues.
     */
    @Override
