...
public class SplayTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void splay(Position<Entry<K, V>> p) {
        ...
        while (!isRoot(current)) {
        	Position<Entry<K, V>> parent = parent(current);
        	Position<Entry<K, V>> grandparent = parent(parent);
        	
        	if (grandparent == null) {
        		rotate(current);
        	} else if ((current.equals(left(parent)) && parent.equals(left(grandparent))
        			|| (current.equals(right(parent)) && parent.equals(right(grandparent))))) {
        		rotate(parent);
        		rotate(current);
        	} else {
        		rotate(current);
