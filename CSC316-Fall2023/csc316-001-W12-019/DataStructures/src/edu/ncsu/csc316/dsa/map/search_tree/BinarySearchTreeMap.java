...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
     ...
     * @param compare Comparator that interacts with K or Key objects 
     */
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public void rotate(Position<Entry<K, V>> p) {
        	BinaryTreeNode<Entry<K, V>> current = validate(p);
        	BinaryTreeNode<Entry<K, V>> parent = current.getParent();
        	...
        	if (grandparent == null) {
        		...
        		if (parent.equals(left(grandparent))) {
        			relink(grandparent, current, true);
        		} else {
        			relink(grandparent, current, false);
        		}
        	}
        	if (current.equals(left(parent))) {
        		relink(parent, current.getRight(), true);
        		relink(current, parent, false);
        	} else {
        		relink(parent, current.getLeft(), false);
        		relink(current, parent, true);
        	}
        }
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
        	BinaryTreeNode<Entry<K, V>> current = validate(x);
        	BinaryTreeNode<Entry<K, V>> parent = current.getParent();
        	BinaryTreeNode<Entry<K, V>> grandparent = parent.getParent();
        	
        	if ((current.equals(parent.getLeft()) && parent.equals(grandparent.getLeft()))
        			|| (current.equals(parent.getRight()) && parent.equals(grandparent.getRight()))) {
        		rotate(parent);
        		return parent;
        	} else {
        		rotate(current);
