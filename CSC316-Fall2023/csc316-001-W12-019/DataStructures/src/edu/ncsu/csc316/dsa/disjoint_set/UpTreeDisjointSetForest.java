...
public class UpTreeDisjointSetForest<E> implements DisjointSetForest<E> {
    ...
    private static class UpTreeNode<E> implements Position<E> {
         ...
         * UpTreeNode
         * 
         * @param count number of nodes contained in the UpTree
         */
    ...
    /**
     * Helper method of find that takes the given UpTreeNode and looks at its parent
     * in search of a node to find.
     * @param current representing the location of the UpTreeNode in the set
     * @return the root node containing the set with the value specified in the find method
     */
    private UpTreeNode<E> findHelper(UpTreeNode<E> current) {
    	if (current.getParent() != current) {
    		current.setParent(findHelper(current.parent));
    	}
