...
public class LinearProbingHashMap<K, V> extends AbstractHashMap<K, V> {
    ...
    public Iterable<Entry<K, V>> entrySet() {
        ...
        for (int i = 0; i < table.length; i++) {
        	// Check that table entry is not null and is not deleted
        	if (!isAvailable(i)) {
    ...
    public V bucketPut(int hash, K key, V value) {
    	int bucket = findBucket(hash, key);
    	if (bucket >= 0) {
    		V original = table[bucket].getValue();
    		table[bucket].setValue(value);
    		return original;
    	} else {
    		table[-bucket - 1] = new TableEntry<K, V>(key, value);
    		size++;
    		return null;
    	}
    }
    ...
    /**
     * Helper method that is used to find the bucket containing the key we are looking for.
     * @param index index into the hash table
     * @param key key of the entry to locate
     * @return the index of the bucket containing the entry if it exists. If the entry is not
     * 		found in the map, return -1(a + 1) where a is the index where the entry should be.
     */
